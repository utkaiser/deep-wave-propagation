...
from model_transformer_old import UTransform as UTransform_old
from model_transformer import UTransform
...
class Model_end_to_end(nn.Module):
    ...
    def __init__(self, param_dict, upsampling_type):
        """
        Parameters
        ----------
        ...
        # define upsampling architecture
        if self.upsampling_type == "UNet3": self.model_upsampling = UNet(wf=1, depth=3, scale_factor=self.param_dict["res_scaler"])
        elif self.upsampling_type == "UTransformer_old": self.model_upsampling = UTransform_old(in_channels=4, scale_factor=self.param_dict["res_scaler"])
        elif self.upsampling_type == "UTransformer": self.model_upsampling = UTransform(in_channels=4, scale_factor=self.param_dict["res_scaler"])
        else: raise NotImplementedError("Architecture not implemented.")
    ...
    def forward(self, x, u_elapse):
        ...
        if self.upsampling_type == "Interpolation":
            ...
            outputs = self.model_upsampling(prop_result.to(device))
...
class Numerical_solver(torch.nn.Module):
    ...
    def forward(self, restr_output, u_elapse):
        ...
        vel_c = torch.Tensor(restr_output[:, 3, :, :])  # b x w_c x h_c

        restr_fine_sol_u, restr_fine_sol_ut = WaveSol_from_EnergyComponent_tensor(
            torch.Tensor(restr_output[:, 0, :, :]).to(device),
            torch.Tensor(restr_output[:, 1, :, :]).to(device),
            torch.Tensor(restr_output[:, 2, :, :]).to(device),
            vel_c.to(device),
            self.c_delta_x,
            torch.sum(torch.sum(torch.Tensor(u_elapse))),
        )
